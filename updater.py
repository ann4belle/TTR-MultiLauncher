"""Updates TTR installation at the folder pointed to."""
import json
import sys
import hashlib
import pathlib
...
def build_update_list(installdir):
...
def do_update(installdir):
    """Main function - automates downloading of required files using multithreading."""
    urls = build_update_list(installdir)
    if not len(urls) > 0:
        return
    ThreadPool(len(urls)).imap_unordered(download, urls)
